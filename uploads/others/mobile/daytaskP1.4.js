...
function createButton1(a, b) {
	...
	c.style.left = buju + 420 + 'px';
	c.style.top = firstpos + 'px';
	c.style.paddingLeft = paddingLeft;
	c.style.paddingRight = paddingRight;
	firstpos = firstpos + delta;
	c.style.width = buttonWidth;
	c.style.height = buttonHeight;
	c.addEventListener('click', b);
	document.body.appendChild(c)
}
function createButton2(a, b) {
	btnList[a] = document.createElement('button');
	var c = btnList[a];
	c.innerText = a;
	c.style.color = color;
	c.style.background = ground;
	c.style.position = 'absolute';
	c.style.left = buju + 365 + 'px';
	c.style.top = secondpos + 'px';
	c.style.paddingLeft = paddingLeft;
	c.style.paddingRight = paddingRight;
...
function func1() {
	...
	function AutoGet1() {
		$("button.cmd_click3").each(function() {
			if (isContains($(this).html(), a)) eval($(this).attr("onclick").replace("look_item corpse", "get corpse"))
		})
	}
}
function func3() {
...
function func6() {
	...
	function fightSwordsman2() {
		$("button.cmd_click3").each(function() {
			if (isContains($(this).html(), a)) eval($(this).attr("onclick").replace("score", "fight"))
		});
		if ($('span.outbig_text:contains(战斗结束)').length > 0) {
			clickButton('prev_combat')
		}
...
function getTianJianTargetCode() {
	...
	for (var i = 0; i < a.length; i++) {
		b = a[i].getAttribute('onclick');
		if (TianJianNPCList.contains(a[i].innerText)) {
			var e = b.split("'")[1].split(" ")[1];
			c[d] = a[i];
			d = d + 1
		}
	...
	if (c.length > 0) {
		...
		console.log("准备杀目标NPC名字：" + c[currentNPCIndex].innerText + "，代码：" + e + "，目标列表中序号：" + (currentNPCIndex));
		clickButton('kill ' + e);
		setTimeout(detectKillTianJianInfo, 200)
	}
}
...
var bangfuNPCList = ["地痞", "镇殿神兽", "守殿神兽", "幽荧幼崽", "幽荧兽魂", "幽荧分身", "幽荧战神", "镇潭神兽", "守潭神兽", "螣蛇幼崽", "螣蛇兽魂", "螣蛇分身", "螣蛇战神", "镇山神兽", "守山神兽", "应龙幼崽", "应龙兽魂", "应龙分身", "应龙战神", "镇谷神兽", "守谷神兽", "饕餮幼崽", "饕餮兽魂", "饕餮王", "饕餮战神", "铁狼军", "银狼军",'金狼军'];
var currentNPCIndex = 0;
...
function getbangfuTargetCode() {
	...
	for (var i = 0; i < a.length; i++) {
		b = a[i].getAttribute('onclick');
		if (bangfuNPCList.contains(a[i].innerText)) {
			var e = b.split("'")[1].split(" ")[1];
			c[d] = a[i];
			d = d + 1
		}
...
function func2_s() {
	...
	if ($('.cmd_click_room')[0].innerText == undefined || $('.cmd_click_room')[0].innerText == "榆叶林" || "青苔石阶") {
		func2_ss = setInterval(func2_go, 2000);
		return
	}
...
function kljfunc() {
	...
	if (btnList["开领奖"].innerText == '开领奖') {
		...
		scanEscapedFish();
		scanEscaped = setInterval(scanEscapedFish, b);
...
function qd() {
	...
	setTimeout(function() {
		go("e;n;e;e;event_1_8041045;event_1_8041045;event_1_44731074;event_1_29721519;e;e;n;lq_lmyh_lb;home;sort fetch_reward;shop money_buy shop1_N_10;cangjian get_all;xueyin_shenbinggu blade get_all;xueyin_shenbinggu unarmed get_all;xueyin_shenbinggu throwing get_all;home；clan buy 302;clan buy 302;clan buy 302;clan buy 302;clan buy 302;clan buy 302;home;jh 5;n;n;e;event_1_1278209;event_1_67345350;jh 2;n;n;n;n;n;n;n;e;tzjh_lq;home;share_ok 1;share_ok 2;share_ok 3;share_ok 4;share_ok 5;share_ok 7")
	}, go_delay * 12)
}
...
function killsnake() {
	if ($('span:contains(胜利)').text().slice(-3) == '胜利！') {
		...
		if (is_fighting) ninesword();
		else clickButton('kill ' + SnakeName)
	}
	setTimeout(killsnake, 500)
}
function mqn() {
...
function XiaKeFunc() {
	if ($('span.outtitle')[0].innerText == "崖底") XuanYaFunc();
	else if ($('span.outtitle')[0].innerText == "石门") {
...
function canyanhua() {
	if ($('span.outtitle')[0].innerText != "阴山岩画") yanhua_go();
	else if ($('span.outtitle')[0].innerText == "阴山岩画") {
...
function shijian() {
	clickButton('swords');
	clickButton('swords select_member huashan_feng');
	clickButton('swords select_member gumu_yangguo');
	clickButton('swords select_member taoist_zhangtianshi');
	clickButton('swords fight_test go');
	setTimeout(Shijian1, 1000)
}
function Shijian1() {
...
function answerQuestions() {
	...
	setTimeout(getAndAnsQuestion, 200)
}
function getAndAnsQuestion() {
	var a = A = $(".out").text().split("题")[1].split("A")[0];
	a = a.replace(/^\theQuestion*/, "");
	a = a.replace(/\theQuestion*$/, "");
	a = a.slice(1);
	var b = getAnswer2Question(a);
	if (b !== "failed") {
		eval("clickButton('question " + b + "')")
	}
...
function getAnswer2Question(a) {
	var b = [];
	var c = 0;
	for (var d in QuestAnsLibs) {
		if (isContains(d, a)) {
			b[c] = d;
			c = c + 1
		} else if (isContains(d, a.replace("npc", "")) || isContains(d, a.replace("NPC", ""))) {}
	}
	if (b.length == 1) {
		return QuestAnsLibs[b[0]]
	} else {
		console.log("题目 " + a + " 找不到答案或存在多个答案，请手动作答！");
		return "failed"
	}
}
function qsd() {
...
function playwarn() {
	var a = new Audio("http://playdreamer.cn/qlyxqj/jijiao.mp3");
	a.play()
}
var dtjname = '';
