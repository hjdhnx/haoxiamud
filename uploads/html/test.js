...
createButton('雪亭街道', btnBox4, XTjiedao);//杀完醉汉杀农夫，再杀完农夫去洛阳
function XTjiedao(){
	go("jh 1;e;n;n;kill snow_drunk;"); 
	p=function(){
	console.log("继续");
	go("s;s;s;w;kill snow_farmer");
	p=function(){
		go("jh 2;n;n;n");
		p=function(){go('jh 3;s;s;s;s;w');}
		setTimeout(checkbusy,5000);
		}
	setTimeout(checkbusy,5000);
	};
	setTimeout(checkbusy,5000);//表示5秒后才执行查忙，因为自己觉得杀个醉汉也要5秒吧
}
